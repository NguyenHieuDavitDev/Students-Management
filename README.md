...
He thong quan ly sinh vien duoc xay dung bang Spring Boot cho bai toan quan ly dao tao (sinh vien, giang vien, lop hoc phan, hoc phi, lich hoc, diem, thong bao...).

## 1. Tong quan

- Backend to chuc theo module package: `com.example.stduents_management.*`.
- Giao dien admin su dung Thymeleaf.
- API JSON cho nghiep vu su dung Spring MVC.
- CSDL SQL Server + migration bang Flyway.
...
- Thymeleaf
- Flyway (`flyway-core`, `flyway-sqlserver`)
...
- Entry point: `com.example.stduents_management.StduentsManagementApplication`.
- Quy uoc:
  - REST API: `*Controller`/`*RestController` (thuong `/api/...`).
  - Dashboard: `*DashboardController` (thuong `/admin/...`).
  - Lich hoc co API noi bo: `/admin/schedules/api`.
...
1. Clone source.
2. Tao database (vi du `students_management`).
3. Cap nhat datasource trong `src/main/resources/application.yml`.
4. Chay build + migration:
   - `mvn clean install`
5. Chay app:
   - `mvn spring-boot:run`
6. Truy cap:
...
### 4.3. Bien cau hinh can kiem tra
...
## 5. Loi thuong gap va cach khac phuc

### 5.1. Sai version Java
- Dau hieu: `UnsupportedClassVersionError`.
- Cach sua: dam bao `java -version` va `mvn -v` deu dung Java 17.

### 5.2. Loi ket noi SQL Server
- Dau hieu: `Cannot open database`, `Login failed for user`, timeout.
- Cach sua: kiem tra SQL Server service, URL JDBC, user/password, quyen DB.

### 5.3. Flyway checksum mismatch
- Dau hieu: `Validate failed: Migration checksum mismatch`.
- Cach sua:
  - Khong sua migration da apply tren moi truong dung chung.
  - Local co the chay `mvn flyway:repair`, sau do `mvn clean install`.

### 5.4. Loi CHECK `session_type`/`SUPPLEMENTARY`
- Dau hieu: tao lich tang cuong bi loi rang buoc.
- Cach sua:
  - Khoi dong lai de Flyway chay migration moi.
  - Neu can, chay tay:
    - `database/migration_schedules_session_type_supplementary.sql`
    - `src/main/resources/db/migration/V20260406140000__schedules_rebuild_check_constraints.sql`

### 5.5. Port 8080 bi chiem
- Dau hieu: `Port 8080 was already in use`.
- Cach sua: dung tien trinh dang dung port hoac doi `server.port`.

### 5.6. IDE khong nhan Lombok
- Dau hieu: bao loi getter/setter du Maven build van pass.
- Cach sua: bat annotation processing, cai plugin Lombok, reload Maven project.

## 6. Ban do module (backend hien tai)

| Package | API | UI |
|---|---|---|
| `attendance` | `/api/attendances` | `/admin/attendances` |
| `auditlog` | - | `/admin/audit-logs` |
| `building` | `/api/buildings` | `/admin/buildings` |
| `classroom` | `/api/classes` | `/admin/classes` |
| `classsection` | `/api/class-sections` | `/admin/class-sections` |
| `course` | `/api/courses` | `/admin/courses` |
| `courseprerequisite` | `/api/course-prerequisites` | `/admin/course-prerequisites` |
| `courseregistration` | - | `/admin/course-registrations` |
| `department` | `/api/departments` | `/admin/departments` |
| `document` | `/api/documents` | `/admin/documents` |
| `educationtype` | `/api/education-types` | `/admin/education-types` |
| `equipment` | `/api/equipments` | `/admin/equipments` |
| `examroom` | - | `/admin/exam-rooms` |
| `examschedule` | - | `/admin/exam-schedules` |
| `examtype` | - | `/admin/exam-types` |
| `faculty` | `/api/faculties` | `/admin/faculties` |
| `feedback` | `/api/feedbacks` | `/admin/feedbacks` |
| `gradecomponent` | - | `/admin/grade-components` |
| `gradescale` | - | `/admin/grade-scales` |
| `graduationcondition` | - | `/admin/graduation-conditions` |
| `graduationresult` | - | `/admin/graduation-results` |
| `lecturer` | `/api/lecturers` | `/admin/lecturers` |
| `lecturercourseclass` | `/api/lecturer-course-classes` | `/admin/lecturer-course-classes` |
| `lecturerduty` | `/api/lecturer-duties` | `/admin/lecturer-duties` |
| `major` | `/api/majors` | `/admin/majors` |
| `notification` | `/api/me/notifications`, `/api/admin/notifications` | `/notifications`, `/admin/notifications` |
| `payment` | - | `/admin/payments` |
| `permission` | - | `/admin/permissions` |
| `position` | `/api/positions` | `/admin/positions` |
| `role` | `/api/roles` | `/admin/roles` |
| `room` | `/api/rooms` | `/admin/rooms` |
| `roomblocktime` | - | `/admin/room-block-times` |
| `roomtype` | `/api/room-types` | `/admin/room-types` |
| `schedule` | `/admin/schedules/api` | `/admin/schedules`, `/admin/schedules/calendar` |
| `scheduleoverride` | - | `/admin/schedule-overrides` |
| `semester` | `/api/semesters` | `/admin/semesters` |
| `student` | `/api/students` | `/admin/students` |
| `studentgrade` | `/api/student-grades` | `/admin/student-grades` |
| `studenttuition` | - | `/admin/student-tuition` |
| `systemsetting` | - | `/admin/settings` |
| `timeslot` | - | `/admin/time-slots` |
| `traininglevel` | `/api/training-levels` | `/admin/training-levels` |
| `trainingprogram` | `/api/training-programs` | `/admin/training-programs` |
| `tuitionfee` | - | `/admin/tuition-fees` |
| `user` | `/api/users` | `/admin/users` |

## 7. Diem nhan module lich hoc

- Ho tro `SessionType.SUPPLEMENTARY` (hoc tang cuong).
- Quy tac nghiep vu: tang cuong co the rut tuan o cuoi cac dong LT/TH cung lop hoc phan.
- `EXAM` khong tinh vao han muc buoi theo tin chi.
- Co calendar keo tha doi lich:
  - doi mot buoi,
  - doi ca lich goc,
  - chi doi phong theo ngay.
- API phuc vu calendar:
  - `GET /admin/schedules/api/meta`
  - `GET /admin/schedules/api/events`
  - `POST /admin/schedules/api/{scheduleId}/room-override`
  - `POST /admin/schedules/api/{scheduleId}/time-slot-override`
  - `POST /admin/schedules/api/{scheduleId}/reschedule-once`

## 8. Ghi chu

- Module `Employee` hien dang o trang thai dac ta trong tai lieu, chua thay package/controller `Employee*` trong snapshot backend hien tai.
- Tai lieu nay uu tien dong bo theo code backend thuc te.
# Students Management System
...
Dự án được phát triển bằng **Spring Boot 3.5.x** , áp dụng kiến trúc RESTful API nhằm hỗ trợ quản lý thông tin sinh viên, lớp học và các nghiệp vụ liên quan một cách khoa học, rõ ràng và dễ mở rộng.
...
Entry point: lớp `com.example.stduents_management.StduentsManagementApplication`; mã nghiệp vụ tổ chức theo package `com.example.stduents_management.*`.

| Thành phần | Phiên bản / ghi chú (theo `pom.xml`) |
|------------|--------------------------------------|
| Java | 17 |
| Spring Boot | 3.5.9 |
| Spring Web, Validation | REST API + bean validation |
| Spring Data JPA | Truy cập CSDL qua repository |
| Spring Security | Xác thực / phân quyền |
| Thymeleaf | Giao diện server-side (`templates/`) |
| Flyway + SQL Server | Migration: `src/main/resources/db/migration/` (`mssql-jdbc`, `flyway-sqlserver`) |
| Lombok | Giảm boilerplate |
| Apache POI (`poi-ooxml` 5.2.5) | Import / export Excel |
| Maven | Build (`artifactId`: `stduents-management`) |

**Quy ước controller trong mã nguồn:**

- REST: class `*Controller` hoặc `*RestController`, thường `@RequestMapping("/api/...")` (JSON cho client / AJAX).
- Giao diện quản trị: `*DashboardController`, thường `@RequestMapping("/admin/...")` (Thymeleaf).
- Một số API nội bộ admin nằm dưới `/admin/.../api` (ví dụ lịch: `ScheduleApiController` → `/admin/schedules/api`).
- Người dùng đăng nhập / đăng ký / quên mật khẩu: `LoginController` (`/login`), `RegisterController` (`/register`), `ForgotPasswordController` (`/forgot-password`); trang chủ `HomeController` (`/`).

Dữ liệu và ràng buộc nghiệp vụ **lấy theo entity/service trong repo**; tài liệu các mục `###` bên dưới mang tính mô tả chi tiết — nếu có lệch, ưu tiên mã Java.

### Bản đồ module (theo package backend)

Bảng tóm tắt **prefix đường dẫn** lấy trực tiếp từ `@RequestMapping` / mapping method trên controller hiện có (cập nhật khi thêm mô-đun).

- **REST:** endpoint JSON (bảo vệ tùy cấu hình Spring Security).
- **Admin / UI:** Thymeleaf dưới `/admin/...`, trừ khi ghi chú khác.

| Package (gốc `...stduents_management.`) | REST API | Admin / UI |
|----------------------------------------|----------|------------|
| `attendance` | `/api/attendances` | `/admin/attendances` |
| `auditlog` | — | `/admin/audit-logs` |
| `building` | `/api/buildings` | `/admin/buildings` |
| `classroom` (lớp niên khóa) | `/api/classes` | `/admin/classes` |
| `classsection` | `/api/class-sections` | `/admin/class-sections` |
| `course` | `/api/courses` | `/admin/courses` |
| `courseprerequisite` | `/api/course-prerequisites` | `/admin/course-prerequisites` |
| `courseregistration` | — | `/admin/course-registrations` |
...
| `lecturerduty` | `/api/lecturer-duties` | `/admin/lecturer-duties` |
| `major` | `/api/majors` | `/admin/majors` |
| `notification` | `GET /api/me/notifications`, `PUT /api/me/notifications/{id}/read`, `POST /api/admin/notifications` | `/notifications`, `/admin/notifications` |
| `payment` | — | `/admin/payments` |
| `permission` | — | `/admin/permissions` |
| `position` | `/api/positions` | `/admin/positions` |
| `role` | `/api/roles` | `/admin/roles` |
...
| `roomblocktime` | — | `/admin/room-block-times` |
| `roomtype` | `/api/room-types` | `/admin/room-types` |
| `schedule` | `/admin/schedules/api` (JSON, `ScheduleApiController`) | `/admin/schedules`, `/admin/schedules/calendar` |
| `scheduleoverride` | — | `/admin/schedule-overrides` |
| `semester` | `/api/semesters` | `/admin/semesters` |
| `student` | `/api/students` | `/admin/students` |
| `studentgrade` | `/api/student-grades` (`StudentGradeRestController`) | `/admin/student-grades` |
| `studenttuition` | — | `/admin/student-tuition` |
| `systemsetting` | — | `/admin/settings` |
| `timeslot` | — | `/admin/time-slots` |
| `traininglevel` | `/api/training-levels` | `/admin/training-levels` |
...
## Hướng dẫn cài đặt và khắc phục lỗi thường gặp

### 1) Yêu cầu môi trường

- **JDK 17** (`java -version` phải là 17).
- **Maven 3.9+** (`mvn -v`).
- **SQL Server** đang chạy, có database cho dự án.
- Cấu hình đúng trong `src/main/resources/application.yml` (host, port, database, user, password).

### 2) Cài đặt nhanh

1. Clone source và vào thư mục dự án.
2. Tạo database trên SQL Server (ví dụ: `students_management`).
3. Cập nhật cấu hình datasource trong `application.yml`.
4. Chạy migration + build:
   - `mvn clean install`
5. Chạy ứng dụng:
   - `mvn spring-boot:run`
6. Mở trình duyệt:
...
#### Lỗi 1: `UnsupportedClassVersionError` / sai phiên bản Java

**Dấu hiệu:** chạy app báo class file version không tương thích.

**Cách xử lý:**
- Cài và chọn JDK 17 làm mặc định.
- Kiểm tra lại bằng `java -version` và `mvn -v`.

#### Lỗi 2: Không kết nối được SQL Server

**Dấu hiệu:** `Cannot open database`, `Login failed for user`, timeout kết nối.

**Cách xử lý:**
- Kiểm tra SQL Server service đã chạy.
- Kiểm tra lại URL JDBC, username/password trong `application.yml`.
- Đảm bảo tài khoản có quyền trên database.
- Nếu chạy Docker/VM, kiểm tra mapping port và firewall.

#### Lỗi 3: Flyway lỗi checksum migration

**Dấu hiệu:** `Validate failed: Migration checksum mismatch`.

**Cách xử lý:**
- Không sửa nội dung migration đã apply trên môi trường dùng chung.
- Ở môi trường local, sau khi xác nhận thay đổi hợp lệ, chạy:
  - `mvn flyway:repair`
- Sau đó chạy lại `mvn clean install`.

#### Lỗi 4: Lỗi ràng buộc `session_type` / `SUPPLEMENTARY`

**Dấu hiệu:** báo lỗi CHECK constraint khi tạo lịch tăng cường.

**Cách xử lý:**
- Khởi động lại ứng dụng để Flyway chạy migration mới nhất.
- Nếu DB cũ chưa đồng bộ, chạy tay script tương ứng trong:
  - `database/migration_schedules_session_type_supplementary.sql`
  - `src/main/resources/db/migration/V20260406140000__schedules_rebuild_check_constraints.sql`
- Kiểm tra lại constraint trên bảng `schedules` trước khi thử lại.

#### Lỗi 5: Port `8080` đã được sử dụng

**Dấu hiệu:** `Port 8080 was already in use`.

**Cách xử lý:**
- Dừng tiến trình đang dùng cổng 8080.
- Hoặc đổi cổng trong `application.yml` (`server.port`).

#### Lỗi 6: Lỗi build Lombok trong IDE

**Dấu hiệu:** IDE báo thiếu getter/setter dù Maven vẫn build được.

**Cách xử lý:**
- Bật annotation processing trong IDE.
- Cài plugin Lombok (nếu IDE yêu cầu).
- Reload project Maven.

### 4) Checklist xác nhận sau cài đặt

- App chạy và mở được `/login`.
- Flyway migrate thành công, không lỗi startup.
- Vào được trang admin cơ bản (`/admin`).
- CRUD thử 1 module (ví dụ Role hoặc Faculty) hoạt động.

## Tính Năng

> **Mới cập nhật:** Xem mô tả chi tiết tại [mục 35 – Quản Lý Phòng Ban](#35-quản-lý-phòng-ban-department-management) và [mục 36 – Quản Lý Chức Vụ Giảng Viên](#36-quản-lý-chức-vụ-giảng-viên-lecturer-duty-management).

> **Tra cứu thống nhất với backend:** Các mục `###` đã được chuẩn hóa thứ tự tăng dần; để đối chiếu nhanh đường dẫn và tên class, dùng [Bản đồ module](#bản-đồ-module-theo-package-backend); chi tiết API lấy theo annotation trên controller trong repo.

### 1. Quản Lý Role (Role Management)

Chức năng quản lý **role** cho phép quản trị viên tạo, sửa, xóa và tra cứu các vai trò trong hệ thống (kết hợp gán quyền qua `User` và module Permissions).

#### Chức năng chính

- **Danh sách**: `GET /api/roles` — toàn bộ role.
- **Tìm kiếm + phân trang**: `GET /api/roles/search?keyword=&page=&size=`.
- **CRUD**: xem chi tiết, tạo, cập nhật, xóa theo ID.

#### API (REST)

`RoleController` — `@RequestMapping("/api/roles")`:

| Method | URL | Mô tả |
|--------|-----|-------|
| GET | `/api/roles` | Danh sách tất cả role |
| GET | `/api/roles/search` | Tìm kiếm + phân trang |
| GET | `/api/roles/{id}` | Chi tiết theo ID |
...
- **RoleDashboardController** — `@RequestMapping("/admin/roles")` (Thymeleaf).

#### Tầng dữ liệu

- Entity **`Role`** (bảng `roles`): `id` (UUID), `name` (unique), `description`.
- **`RoleService`**: `getAll`, `search`, `getById`, `create`, `update`, `delete`.

DTO: `role.dto.RoleRequest`, `RoleResponse`.

### 2. Quản lý điểm sinh viên (Student Grades)
...
#### Nghiệp vụ nhập điểm (đúng vai trò giảng viên)
...
`StudentGradeRestController` (REST, prefix `/api/student-grades`):
...
### 3. Quản lý cấu hình học phí (Tuition Fees)
...
### 4. Quản lý học phí sinh viên (Student Tuition)
...
### 5. Quản lý lịch sử thanh toán học phí (Payments)
...
#### Nghiệp vụ liên quan tuition_fees và student_tuition
...
### 6. Quản lý loại kỳ thi (Exam Types)
...
### 9. Quản Lý Khoa (Faculty Management)
...
### 10. Quản Lý Ngành Học (Major Management)
...
### 11. Quản Lý Lớp Học (Classroom Management)
...
### 12. Quản Lý Loại Đào Tạo (Education Type Management)
...
### 13. Quản Lý Bậc Đào Tạo (Training Level Management)
...
### 14. Quản Lý Sinh Viên (Student Management)
...
### 15. Quản Lý Giảng Viên (Lecturer Management)
...
### 16. Quản Lý Chức Danh (Position Management)
...
### 17. Quản Lý học phần (Course Management)
...
### 18. Quản Lý Toà Nhà (Building Management)
...
### 19. Quản Lý Loại Phòng (Room Type Management)
...
### 20. Quản Lý Học Phần Tiên Quyết (Course Prerequisite Management)
...
### 21. Quản Lý Thiết Bị (Equipment Management)
...
### 22. Quản Lý Phân Công Giảng Viên (Lecturer Course Class Assignment Management)
...
### 23. Đăng Ký Học Phần (Course Registration Management)
...
### 24. Quản Lý Khung Giờ (Time Slot Management)
...
### 25. Quản Lý Lịch Học (Schedule Management)
...
| `sessionType` | Enum | Loại buổi: `THEORY` (lý thuyết), `PRACTICE` (thực hành), `EXAM` (thi / kiểm tra — **không tính** vào hạn mức buổi theo tín chỉ), `SUPPLEMENTARY` (**tăng cường** — xem mục dưới) |
...
#### Lịch theo tuần và kéo thả đổi lịch (Calendar)

- **Trang lịch tuần** (`GET /admin/schedules/calendar`): xem thời khóa biểu dạng lưới theo **học kỳ** và **khoảng ngày**; dữ liệu sự kiện được nạp qua API JSON (metadata + danh sách buổi).
- **Kéo thả**: kéo thẻ một buổi học sang ô khác (ngày / tiết). Sau khi thả, hệ thống mở hộp thoại **“Xác nhận thay đổi sau khi kéo thả”** với các chế độ:
  - **Chỉ buổi này** (mặc định): chỉ thay đổi **đúng ngày** tương ứng thao tác kéo; dịch theo **thứ / tiết** của ô đích; **các tuần và buổi khác** của cùng dòng lịch **giữ nguyên**. Thay đổi được ghi nhận qua cơ chế **override** (tương thích module [Dạy bù / Đổi phòng](#26-quản-lý-thay-đổi-lịch---dạy-bù--đổi-phòng-schedule-overrides)).
  - **Đổi cả lịch gốc**: cập nhật **thứ và tiết** cho **cả chuỗi lặp** của dòng lịch (gọi cập nhật lịch gốc); dùng khi muốn đổi lịch cố định, không chỉ một ngày. Có thể chọn **phòng** áp dụng cho dòng lịch sau khi lưu.
  - **Chỉ đổi phòng**: **giữ nguyên** thứ, tiết và tuần; chỉ ghi nhận **phòng mới** cho **đúng buổi ngày** đang kéo (bản ghi kiểu đổi phòng theo ngày).
- **API phục vụ lịch & kéo thả** (`ScheduleApiController`, prefix `/admin/schedules/api`):
  - `GET /admin/schedules/api/meta` — metadata phục vụ giao diện (khung giờ, phòng, …).
  - `GET /admin/schedules/api/events?semesterId=&start=&end=` — danh sách sự kiện trong khoảng ngày.
  - `POST /admin/schedules/api/{scheduleId}/room-override` — đổi phòng cho **một ngày** (không sửa dòng lịch gốc).
  - `POST /admin/schedules/api/{scheduleId}/time-slot-override` — đổi **tiết** cho **một ngày** (cùng ngày trên lịch).
  - `POST /admin/schedules/api/{scheduleId}/reschedule-once` — **dịch một buổi** sang **ngày + tiết** đích; lịch gốc không đổi.
  - CRUD JSON: `GET/POST/PUT/DELETE /admin/schedules/api/...` cho tạo/sửa/xóa dòng lịch (dùng khi chế độ “đổi cả lịch gốc” hoặc tích hợp khác).

#### Học tăng cường (loại buổi `SUPPLEMENTARY`)

- Ngoài buổi **lý thuyết**, **thực hành** và **thi / kiểm tra**, hệ thống hỗ trợ loại buổi **`SUPPLEMENTARY` (Tăng cường)** trên lịch học (form Thymeleaf và payload API).
- **Quy tắc nghiệp vụ khi lưu**: với mỗi **buổi lặp theo tuần** thuộc dòng **tăng cường** (theo khoảng tuần và mẫu tuần `weekPattern`), backend **tự rút** số **tuần tương ứng** từ **phần cuối** các dòng **lý thuyết** hoặc **thực hành** **cùng lớp học phần** — tức “**tăng cường thay buổi cuối**” của chuỗi LT/TH, thay vì cộng thêm tuyệt đối không điều chỉnh.
- **Hạn mức theo tín chỉ**: tổng **buổi lặp theo tuần** (lý thuyết + thực hành + tăng cường) cho học phần vẫn nằm trong giới hạn **5 × số tín chỉ**; **buổi `EXAM` không tính** vào hạn mức này. Nếu vượt hạn, API trả lỗi nghiệp vụ kèm gợi ý (rút ngắn tuần, bớt tăng cường, hoặc dùng loại thi khi phù hợp).
...
- **ScheduleDashboardController**: Quản lý views HTML cho giao diện web tại `/admin/schedules` (CRUD, Import/Export/Print, Auto Schedule, **lịch tuần** `/admin/schedules/calendar`).
- **ScheduleApiController**: REST nội bộ dưới `/admin/schedules/api` — metadata/sự kiện lịch, CRUD JSON, và các endpoint **override / reschedule một buổi** phục vụ **kéo thả** trên trang calendar.
- **ScheduleApiExceptionHandler**: Chuẩn hóa thông báo lỗi API lịch (kể cả gợi ý khi DB chưa cập nhật CHECK cho `SUPPLEMENTARY`).

---

### 26. Quản Lý Thay Đổi Lịch - Dạy Bù / Đổi Phòng (Schedule Overrides)
...
### 27. Quản Lý Permissions (Gán quyền cho vai trò)
...
### 28. Quản Lý Khóa Phòng (Room Block Times)
...
### 29. Quản Lý Thành Phần Điểm (Grade Components)

Module **grade_components** định nghĩa các thành phần cấu thành điểm tổng kết của một lớp học phần (ví dụ: điểm giữa kỳ, điểm cuối kỳ, điểm thực hành,…), kèm trọng số (%) và điểm tối đa. Đây là dữ liệu nền để giảng viên nhập điểm (`student_grades`) và service tính điểm tổng kết (`GradeTranscriptService`).
...
| `course_class_id` | FK → `class_sections` | Lớp học phần sở hữu thành phần này |
| `component_name` | NVARCHAR(100) | Tên thành phần điểm (bắt buộc) |
| `weight` | DECIMAL(5,2) | Trọng số (%), ví dụ: 50 = 50% |
| `max_score` | DECIMAL(4,2) | Điểm tối đa của thành phần |
| `created_at` | DATETIME | Thời điểm tạo (tự động) |

#### Tính năng

- **CRUD**: Thêm, sửa, xóa thành phần điểm cho từng lớp học phần.
- **Tìm kiếm gần đúng** (case-insensitive): theo tên thành phần, mã lớp học phần, tên lớp, mã/tên học phần.
- **Phân trang**: mặc định 10/trang, sắp xếp theo `createdAt` giảm dần.
- **Import Excel**: Nhập hàng loạt từ file `.xlsx` với cấu trúc 4 cột:
  - Mã lớp học phần → tìm `ClassSection` theo `classCode`
  - Tên thành phần điểm (bắt buộc)
  - Trọng số (%)
  - Điểm tối đa
  - Bỏ qua dòng thiếu mã lớp hoặc tên thành phần; báo lỗi dòng cụ thể nếu không tìm thấy lớp.
- **Export Excel**: Xuất toàn bộ ra file `grade-components.xlsx` (4 cột tương ứng).
- **In**: Trang in `grade-components/print.html` hiển thị danh sách đầy đủ, tự động gọi `window.print()`.

#### Service (GradeComponentService)

| Phương thức | Mô tả |
|-------------|-------|
| `search(keyword, page, size)` | Tìm kiếm phân trang, sắp xếp mới nhất trước |
| `getById(id)` | Lấy chi tiết, ném 404 nếu không tồn tại |
| `create(req)` / `update(id, req)` | CRUD, kiểm tra `ClassSection` tồn tại |
| `delete(id)` | Xóa, kiểm tra tồn tại trước |
| `getForPrint()` | Lấy toàn bộ để in |
| `importExcel(file)` | Đọc và lưu từng dòng Excel, báo lỗi row cụ thể |
| `exportExcel()` | Tạo file `.xlsx` trả về `byte[]` |

#### Giao diện Admin (URL)

| Chức năng | URL |
|-----------|-----|
| Danh sách | `GET /admin/grade-components` |
...
| Import Excel | `POST /admin/grade-components/import` |
| Export Excel | `GET /admin/grade-components/export` |

#### Cấu trúc code

```
gradecomponent/
├── controller/GradeComponentDashboardController.java   # Thymeleaf CRUD + Import/Export/Print
├── dto/
│   ├── GradeComponentRequest.java    # courseClassId, componentName, weight, maxScore (@Valid)
│   └── GradeComponentResponse.java  # record: id, courseClassId, classCode, className, courseCode, courseName, componentName, weight, maxScore, createdAt
├── entity/GradeComponent.java        # bảng grade_components, @ManyToOne ClassSection
├── repository/GradeComponentRepository.java  # searchByKeyword, findByClassSection_Id...
└── service/GradeComponentService.java
```

**Templates**: `grade-components/index.html`, `form.html`, `print.html`

### 30. Quản Lý Điều Kiện Xét Tốt Nghiệp (Graduation Conditions)

Module **graduation_conditions** quản lý các tiêu chí xét tốt nghiệp áp dụng cho từng **chương trình đào tạo**. Mỗi chương trình chỉ có **một bộ điều kiện** (ràng buộc 1-1); hệ thống kiểm tra xung đột khi tạo/sửa.
...
| `program_id` | FK → `training_programs` | Chương trình đào tạo áp dụng |
| `min_credits` | INT | Số tín chỉ tối thiểu cần tích lũy |
| `min_gpa` | DECIMAL(4,2) | Điểm trung bình tích lũy tối thiểu (hệ 4) |
| `required_certificate` | NVARCHAR(500) | Chứng chỉ bắt buộc (ngoại ngữ, tin học,…) |
| `required_courses` | NVARCHAR(MAX) | Danh sách học phần bắt buộc (mỗi dòng 1 mã/mô tả) |
| `created_at` | DATETIME | Tự động ghi khi tạo |
| `updated_at` | DATETIME | Tự động cập nhật khi sửa |

#### Nghiệp vụ chính

- **Ràng buộc “1 chương trình – 1 điều kiện”**:
  - Khi tạo: kiểm tra `findByProgram_ProgramId(programId)` — nếu đã tồn tại → HTTP 409 Conflict.
  - Khi sửa: kiểm tra `existsByProgram_ProgramIdAndIdNot(programId, id)` — không cho đổi sang chương trình đã có điều kiện khác.
- **Form thêm mới** chỉ dropdown các chương trình đang hoạt động (`isActive = true`) **chưa có** điều kiện xét tốt nghiệp.
- **Form chỉnh sửa** hiển thị chương trình hiện tại của bản ghi và toàn bộ chương trình đang hoạt động để cho phép đổi nếu cần.
- Có thể tra cứu điều kiện theo `programId` qua `getByProgramId(UUID)` (dùng cho tích hợp kiểm tra tốt nghiệp sau này).

#### Service (GraduationConditionService)

| Phương thức | Mô tả |
|-------------|-------|
| `search(keyword, page, size)` | Tìm kiếm phân trang theo `programCode`, `programName`, `requiredCertificate`; sắp xếp theo tên CTĐT |
| `getById(id)` | Lấy chi tiết, ném 404 nếu không tồn tại |
| `getAll()` | Lấy toàn bộ (dùng cho print) |
| `getByProgramId(programId)` | Tìm điều kiện của một CTĐT (null nếu chưa có) |
| `create(req)` | Tạo mới, kiểm tra không trùng CTĐT |
| `update(id, req)` | Sửa, kiểm tra không xung đột CTĐT |
| `delete(id)` | Xóa, kiểm tra tồn tại trước |
...
**`GraduationConditionRequest`**:

| Trường | Ràng buộc | Mô tả |
|--------|-----------|-------|
| `programId` | `@NotNull` | UUID chương trình đào tạo |
| `minCredits` | `@Min(0)` `@Max(999)` | Số tín chỉ tối thiểu |
| `minGpa` | `0.00 – 4.00`, `@Digits(1,2)` | GPA tối thiểu |
| `requiredCertificate` | `@Size(max=500)` | Chứng chỉ bắt buộc |
| `requiredCourses` | _(tùy chọn)_ | Học phần bắt buộc (text tự do) |

**`GraduationConditionResponse`** (record): `id`, `programId`, `programCode`, `programName`, `majorName`, `course`, `minCredits`, `minGpa`, `requiredCertificate`, `requiredCourses`, `createdAt`, `updatedAt`.

#### Cấu trúc code

```
graduationcondition/
├── controller/GraduationConditionDashboardController.java   # Thymeleaf CRUD
├── dto/
│   ├── GraduationConditionRequest.java
│   └── GraduationConditionResponse.java
├── entity/GraduationCondition.java    # bảng graduation_conditions, @ManyToOne TrainingProgram
├── repository/GraduationConditionRepository.java  # search, findAllOrdered, findByProgram_ProgramId
└── service/GraduationConditionService.java
```
...
### 31. Quản Lý Thông Báo (Notification Management)

Module **Thông Báo** cho phép hệ thống gửi, quản lý và hiển thị thông báo đến từng người dùng (sinh viên / giảng viên / quản trị viên). Hỗ trợ cả **tạo thủ công** (admin) lẫn **tự động theo sự kiện** (upsert từ các module nghiệp vụ như lịch thi, học phí, lịch học…).
...
| `recipient_user_id` | UUID (FK → `users`) | Người nhận thông báo |
| `category` | VARCHAR(50) (ENUM) | Danh mục thông báo |
...
| `scheduled_at` | DATETIME | Thời gian dự kiến hiển thị |
| `source_type` | VARCHAR(60) | Loại nguồn sự kiện (dùng cho auto CRUD) |
| `source_id` | VARCHAR(120) | ID nguồn sự kiện |
| `created_by` | UUID | Admin/system đã tạo thông báo |

**Ràng buộc duy nhất**: `(recipient_user_id, category, source_type, source_id)` — đảm bảo mỗi nguồn sự kiện chỉ tạo đúng một thông báo cho mỗi người nhận (dùng cho upsert tự động). Với thông báo tạo tay, `source_type`/`source_id` là `null` nên không bị ràng buộc trùng.

#### Danh mục thông báo (NotificationCategory)

| Giá trị (enum) | Nhãn hiển thị |
|----------------|---------------|
...
#### Nghiệp vụ chính (NotificationService)

| Phương thức | Mô tả |
|-------------|-------|
| `createForUserId(...)` | Tạo thông báo thủ công cho một user cụ thể (có thể kèm `scheduledAt`, `createdBy`) |
| `createManual(req)` | Tạo thông báo từ form admin (BindingResult validation) |
| `updateManual(id, req)` | Sửa thông báo tạo tay; tự động xóa `sourceType`/`sourceId` để tránh bị ghi đè bởi auto CRUD |
| `deleteManual(id)` | Xóa thông báo, kiểm tra tồn tại trước |
| `upsertForUserBySource(...)` | Auto CRUD: nếu đã có thông báo cho cùng `(user, category, sourceType, sourceId)` thì **update** title/content/scheduledAt; ngược lại **tạo mới**. Không reset `read`/`readAt` |
| `deleteBySource(sourceType, sourceId)` | Xóa tất cả thông báo thuộc một nguồn sự kiện (dùng khi sự kiện bị hủy) |
| `searchForCurrentUser(category, unreadOnly, pageable)` | Tìm kiếm thông báo cho người dùng đang đăng nhập, lọc theo danh mục và trạng thái đọc |
| `searchForAdmin(category, recipientUserId, unreadOnly, keyword, pageable)` | Tìm kiếm toàn bộ thông báo dành cho admin, hỗ trợ lọc đa tiêu chí và keyword |
| `markAsRead(notificationId)` | Đánh dấu thông báo là đã đọc (ghi `readAt`); chỉ áp dụng cho thông báo của chính user đang đăng nhập |
| `countUnreadForCurrentUser()` | Đếm số thông báo chưa đọc của user hiện tại (dùng cho badge trên header) |
| `getById(id)` | Lấy chi tiết một thông báo (dùng cho form sửa) |

**Cơ chế Auto CRUD**: Các module nghiệp vụ (lịch thi, lịch học, học phí…) gọi `upsertForUserBySource()` để đồng bộ thông báo mà không tạo trùng. Khi sự kiện bị xóa, gọi `deleteBySource()` để dọn dẹp thông báo liên quan.

#### API Endpoints (REST)

`NotificationRestController`:

| Method | URL | Phân quyền | Mô tả |
|--------|-----|-----------|-------|
| `GET` | `/api/me/notifications` | Đã đăng nhập | Lấy danh sách thông báo của user hiện tại (lọc theo `category`, `unreadOnly`, phân trang) |
| `PUT` | `/api/me/notifications/{id}/read` | Đã đăng nhập | Đánh dấu một thông báo là đã đọc |
| `POST` | `/api/admin/notifications` | `ROLE_ADMIN` | Tạo thông báo cho user bất kỳ (tạo thủ công) |

**Query params** (`GET /api/me/notifications`):

| Tham số | Mặc định | Mô tả |
|---------|----------|-------|
| `category` | _(tất cả)_ | Lọc theo danh mục enum |
| `unreadOnly` | `false` | Chỉ lấy thông báo chưa đọc |
| `page` | `0` | Trang hiện tại |
| `size` | `10` | Số bản ghi mỗi trang |

#### Giao diện người dùng (User Dashboard)

`NotificationDashboardController` (prefix `/notifications`):

- **`GET /notifications`** — Trang danh sách thông báo của user đang đăng nhập:
  - Lọc theo danh mục và trạng thái đọc/chưa đọc.
  - Phân trang, hiển thị tiêu đề, nội dung, danh mục, thời gian tạo, badge "Chưa đọc".
  - Template: `notifications/index.html`.

- **`POST /notifications/{id}/read`** — Đánh dấu thông báo đã đọc, sau đó redirect về `/notifications` với flash message thành công.

#### Giao diện quản trị (Admin Dashboard)

`NotificationAdminDashboardController` (prefix `/admin/notifications`):

- **`GET /admin/notifications`** — Danh sách toàn bộ thông báo trong hệ thống:
  - Lọc theo: danh mục, người nhận (`recipientUserId`), trạng thái đọc, từ khóa (tìm trong tiêu đề và nội dung).
  - Phân trang.
  - Template: `notifications/admin/index.html`.

- **`GET /admin/notifications/new`** — Form tạo thông báo mới:
  - Chọn **người nhận** (dropdown danh sách toàn bộ user, sắp xếp theo username).
  - Chọn **danh mục** thông báo.
  - Nhập **tiêu đề** và **nội dung**.
  - Chọn **thời gian dự kiến** (`scheduledAt`, mặc định là thời điểm hiện tại).
  - Template: `notifications/admin/form.html` (mode `create`).

- **`POST /admin/notifications`** — Lưu thông báo mới; nếu validation lỗi thì hiển thị lại form.

- **`GET /admin/notifications/{id}/edit`** — Form chỉnh sửa thông báo:
  - Load dữ liệu hiện tại vào form.
  - Template: `notifications/admin/form.html` (mode `edit`).

- **`POST /admin/notifications/{id}`** — Cập nhật thông báo; sau khi thành công redirect về danh sách với flash message "Cập nhật thông báo thành công".

- **`POST /admin/notifications/{id}/delete`** — Xóa thông báo; sau khi thành công redirect về danh sách với flash message "Xóa thông báo thành công".

#### Badge thông báo trên Header

`NotificationHeaderModelAdvice` — `@ControllerAdvice` tự động inject số thông báo chưa đọc vào mọi view:

- Gọi `notificationService.countUnreadForCurrentUser()` cho mọi request của user đã đăng nhập.
- Đặt kết quả vào model attribute `unreadNotificationCount`.
- Dùng để hiển thị badge số trên icon thông báo ở header/navbar.
- Tự động bỏ qua nếu user chưa đăng nhập (xử lý `ResponseStatusException` 401).

#### DTO

**`NotificationCreateRequest`** (tạo/sửa):

| Trường | Ràng buộc | Mô tả |
|--------|-----------|-------|
| `recipientUserId` | `@NotNull` | UUID người nhận |
| `category` | `@NotNull` | Danh mục thông báo |
| `title` | `@NotBlank` | Tiêu đề |
| `content` | `@NotBlank` | Nội dung |
| `scheduledAt` | _(tùy chọn)_ | Thời gian dự kiến |

**`NotificationResponse`** (record trả về từ API/service):
...
record NotificationResponse(
    ...
    String categoryLabel,       // Nhãn tiếng Việt
    String title,
    String content,
...
| Method | Mô tả |
|--------|-------|
| `searchForUser(userId, category, unreadOnly, pageable)` | Tìm thông báo của một user, lọc theo danh mục và trạng thái đọc, sắp xếp mới nhất trước |
| `searchForAdmin(category, recipientUserId, unreadOnly, keyword, pageable)` | Tìm kiếm toàn bộ với nhiều tiêu chí, keyword so khớp với title và content (case-insensitive) |
| `findByIdAndRecipientUser_Id(id, userId)` | Tìm thông báo theo ID và chủ sở hữu (bảo vệ quyền truy cập) |
| `findLatestBySource(userId, category, sourceType, sourceId)` | Tìm thông báo mới nhất theo nguồn sự kiện (dùng cho upsert) |
| `countByRecipientUser_IdAndReadFalse(userId)` | Đếm thông báo chưa đọc của user |
| `deleteBySourceTypeAndSourceId(sourceType, sourceId)` | Xóa theo nguồn sự kiện khi sự kiện bị hủy |

#### Tích hợp với các module khác

Các module nghiệp vụ có thể tích hợp thông báo tự động bằng cách gọi `NotificationService`:

- **Lịch thi** (`ExamScheduleService`): gọi `upsertForUserBySource()` với `category = EXAM_SCHEDULE` khi tạo/cập nhật lịch thi; gọi `deleteBySource()` khi hủy lịch thi.
- **Học phí** (`PaymentService`): gọi `createForUserId()` với `category = TUITION_FEE` khi ghi nhận thanh toán hoặc phát sinh công nợ.
- **Thay đổi lịch học** (`ScheduleOverrideService`): gọi `upsertForUserBySource()` với `category = SCHEDULE_CHANGE` khi có dạy bù / đổi phòng.
...
notification/
├── controller/
│   ├── NotificationAdminDashboardController.java   # Admin CRUD (Thymeleaf)
│   ├── NotificationDashboardController.java        # User xem thông báo (Thymeleaf)
│   ├── NotificationHeaderModelAdvice.java          # Badge chưa đọc (ControllerAdvice)
│   └── NotificationRestController.java             # REST API
├── dto/
│   ├── NotificationCreateRequest.java              # Form tạo/sửa
│   └── NotificationResponse.java                  # Response record
├── entity/
│   ├── Notification.java                           # Entity bảng notifications
│   └── NotificationCategory.java                  # Enum danh mục
├── repository/
...
**Templates** (`resources/templates/notifications/`):
- `index.html` — Trang thông báo cho user
- `admin/index.html` — Danh sách thông báo cho admin
- `admin/form.html` — Form tạo/sửa thông báo

---

### 32. Quản Lý Thang Điểm & Bảng Điểm Tổng Kết (Grade Scale & Transcript)

Module **gradescale** gồm hai chức năng liên quan chặt chẽ:

1. **Quản lý thang điểm** (`grade_scales`) — Định nghĩa bảng quy đổi điểm số (hệ 10) → điểm chữ + điểm GPA (hệ 4) + xếp loại.
2. **Xem bảng điểm tổng kết** — Tính và hiển thị điểm tổng kết của từng sinh viên trong một lớp học phần theo quy trình 4 bước.
...
|-----|------|-------|
| `id` | UUID (PK) | Khóa chính, tự sinh |
| `min_score` | DECIMAL(4,2) | Điểm số tối thiểu áp dụng mức này |
| `max_score` | DECIMAL(4,2) | Điểm số tối đa áp dụng mức này |
| `letter_grade` | VARCHAR(2) | Điểm chữ (A, B+, B, C+, C, D+, D, F…) — **duy nhất** |
| `grade_point` | DECIMAL(3,2) | Điểm GPA hệ 4 (0.00 – 4.00) |
| `description` | NVARCHAR(100) | Xếp loại (Xuất sắc, Giỏi, Khá,…) |

**Ràng buộc**: `letter_grade` phải duy nhất; `minScore < maxScore`; GPA trong khoảng 0–4.

#### Quy trình tính điểm tổng kết (4 bước)

```
Bước 1  Admin tạo GradeComponent (có weight %) cho lớp học phần
Bước 2  Giảng viên nhập StudentGrade (điểm từng thành phần)
Bước 3  GradeTranscriptService tính: totalScore = ∑(score_i × weight_i / 100)
Bước 4  Tra grade_scales → letterGrade, gradePoint (GPA), classification
```

#### Service

**`GradeScaleService`** — CRUD thang điểm:

| Phương thức | Mô tả |
|-------------|-------|
| `search(keyword, page, size)` | Tìm kiếm theo `letterGrade` / `description`, sắp xếp `minScore` giảm dần |
| `getById(id)` | Lấy chi tiết, ném 404 nếu không có |
| `getAll()` | Lấy toàn bộ (dùng cho print và hiển thị bảng tra) |
| `create(req)` | Tạo mới, validate `minScore < maxScore` và `letterGrade` không trùng |
| `update(id, req)` | Sửa, validate tương tự (loại trừ chính id đang sửa) |
| `delete(id)` | Xóa, kiểm tra tồn tại |
| `convert(score)` | Tra bảng `grade_scales` theo điểm số → trả về `Optional<GradeScaleResponse>` (dùng bởi `GradeTranscriptService`) |

**`GradeTranscriptService`** — Tính bảng điểm tổng kết:

| Phương thức | Mô tả |
|-------------|-------|
| `calculateForClass(courseClassId)` | Tính điểm tổng kết cho **toàn bộ sinh viên** trong lớp (batch-load tránh N+1); trả về `List<StudentTranscriptResult>` sắp xếp theo tên SV |
| `calculateForStudent(studentId, courseClassId)` | Tính điểm tổng kết cho **một sinh viên** trong lớp |
| `getAllClasses()` | Lấy danh sách lớp học phần (cho dropdown selector) |
| `getComponents(courseClassId)` | Lấy danh sách thành phần điểm của lớp (cho header bảng) |

**Chiến lược batch-load** trong `calculateForClass`:
1. Load toàn bộ `GradeComponent` của lớp một lần.
2. Load toàn bộ `StudentGrade` của lớp một lần, nhóm thành `Map<studentId, Map<componentId, StudentGrade>>`.
3. Duyệt từng `CourseRegistration` của lớp để tạo kết quả.

#### DTO

**`GradeScaleRequest`** (tạo/sửa):

| Trường | Ràng buộc | Mô tả |
|--------|-----------|-------|
| `minScore` | `@NotNull`, `0–100` | Điểm tối thiểu |
| `maxScore` | `@NotNull`, `0–100` | Điểm tối đa |
| `letterGrade` | `@NotBlank`, `@Size(max=2)` | Điểm chữ |
| `gradePoint` | `@NotNull`, `0–4` | GPA |
| `description` | `@Size(max=100)` | Xếp loại |

**`GradeScaleResponse`** (record): `id`, `minScore`, `maxScore`, `letterGrade`, `gradePoint`, `description`.

**`GradeLineDetail`** (record — chi tiết một thành phần trong bảng điểm):
- `gradeComponentId`, `componentName`, `weight` (%), `maxScore`, `score` (null nếu chưa nhập), `weightedScore` (= score × weight/100).

**`StudentTranscriptResult`** (record — kết quả một sinh viên):

| Trường | Mô tả |
|--------|-------|
| `studentCode`, `studentName` | Thông tin sinh viên |
| `classCode`, `className`, `courseName` | Thông tin lớp học phần |
| `gradeLines` | `List<GradeLineDetail>` — chi tiết từng thành phần điểm |
| `totalScore10` | Điểm tổng kết hệ 10 (null nếu chưa nhập đủ điểm) |
| `gradePoint` | GPA hệ 4 (từ `grade_scales`) |
| `letterGrade` | Điểm chữ (từ `grade_scales`) |
| `classification` | Xếp loại (description từ `grade_scales`) |
| `scaleFound` | `true` nếu tìm được mức thang điểm phù hợp |
| `fullyGraded` | `true` nếu đã nhập đủ điểm tất cả thành phần |

#### Giao diện Admin (URL)

**Quản lý thang điểm** (`GradeScaleDashboardController`, prefix `/admin/grade-scales`):

| Chức năng | URL |
|-----------|-----|
| Danh sách thang điểm | `GET /admin/grade-scales` |
| Thêm mới | `GET /admin/grade-scales/new` · `POST /admin/grade-scales` |
| Sửa | `GET /admin/grade-scales/{id}/edit` · `POST /admin/grade-scales/{id}` |
| Xóa | `POST /admin/grade-scales/{id}/delete` |
| In bảng thang điểm | `GET /admin/grade-scales/print` |

**Bảng điểm tổng kết** (cùng controller):

| Chức năng | URL |
|-----------|-----|
| Xem bảng điểm theo lớp | `GET /admin/grade-scales/transcript?courseClassId={id}` |
| In bảng điểm theo lớp | `GET /admin/grade-scales/transcript/print?courseClassId={id}` |

**Trang `transcript`** hiển thị:
- Dropdown chọn lớp học phần.
- Bảng điểm với cột động: STT · Mã SV · Họ tên · [từng thành phần điểm] · Tổng kết · Điểm chữ · GPA · Xếp loại.
- **Thống kê nhanh**: tổng số sinh viên, số đạt (≥ 5.0), số rớt.
- Nút **In bảng điểm** dẫn đến trang in.

**Validation lỗi**:
- `minScore >= maxScore` → HTTP 400.
- `letterGrade` trùng → HTTP 409 Conflict.

#### Repository (GradeScaleRepository)

| Method | Mô tả |
|--------|-------|
| `searchByKeyword(keyword, pageable)` | Tìm theo `letterGrade` / `description` (case-insensitive) |
| `findByScore(score)` | Tìm mức thang điểm áp dụng cho điểm số cho trước (`minScore ≤ score ≤ maxScore`) |
| `existsByLetterGrade(letterGrade)` | Kiểm tra trùng điểm chữ khi tạo mới |
| `existsByLetterGradeAndIdNot(letterGrade, id)` | Kiểm tra trùng điểm chữ khi sửa |

#### Cấu trúc code

```
gradescale/
├── controller/GradeScaleDashboardController.java   # CRUD thang điểm + trang transcript
├── dto/
│   ├── GradeScaleRequest.java          # form tạo/sửa thang điểm
│   ├── GradeScaleResponse.java         # record trả về
│   ├── GradeLineDetail.java            # chi tiết một thành phần trong bảng điểm
│   └── StudentTranscriptResult.java    # kết quả bảng điểm một sinh viên
├── entity/GradeScale.java              # bảng grade_scales
├── repository/GradeScaleRepository.java
└── service/
    ├── GradeScaleService.java          # CRUD + convert(score)
    └── GradeTranscriptService.java     # calculateForClass, calculateForStudent
```

**Templates**: `grade-scales/index.html`, `form.html`, `print.html`, `transcript.html`, `transcript-print.html`

---

### 33. Quản Lý Phản Hồi Đánh Giá Giảng Viên (Feedback Management)
...
#### Giao diện Admin — Thymeleaf (`/admin/feedbacks`)
...
#### Repository (TeachingFeedbackRepository)
...
### 34. Quản Lý Điểm Danh Sinh Viên (Attendance Management)
...
#### Giao diện Admin — Thymeleaf (`/admin/attendances`)
...
### 35. Quản Lý Phòng Ban (Department Management)
...
#### Giao diện Admin (`/admin/departments`)
...
### 36. Quản Lý Chức Vụ Giảng Viên (Lecturer Duty Management)
...
#### Giao diện Admin (`/admin/lecturer-duties`)
...
### 37. Nhật Ký Hoạt Động (Audit Log)
...
#### Giao diện Admin (`/admin/audit-logs`)
...
### 38. Quản Lý Nhân Viên (Employee Management)

> **Đồng bộ với repo:** Trong mã nguồn hiện tại **không** có package/controller/service `Employee*`; các URL `/api/employees`, `/admin/employees` và toàn bộ chi tiết dưới đây là **đặc tả / kế hoạch** trong README. Khi triển khai, cần thêm mô-đun theo quy ước trong [Kiến trúc backend & công nghệ](#kiến-trúc-backend--công-nghệ) và bổ sung dòng tương ứng vào [Bản đồ module](#bản-đồ-module-theo-package-backend).
...
#### Giao diện Admin (`/admin/employees`)
...
- [x] Quản lý thang điểm & bảng điểm tổng kết (Grade Scale & Transcript)
